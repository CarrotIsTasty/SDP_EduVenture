...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['changeRole'])) {
    ...
    $stmt = $conn->prepare("UPDATE user SET Role = 'lecturer' WHERE userID = ?");
    $stmt->bind_param("s", $userID);
    
    if ($stmt->execute()) {
        echo "<script>alert('Role changed to lecturer successfully!');</script>";
    } else {
        echo "<script>alert('Error: " . $stmt->error . "');</script>";
    }
...
$sql = "SELECT Name, userID, Age FROM user WHERE Role='student'";
                    ...
                    while ($row = $result->fetch_assoc()) {
                        echo "<tr>
                                <td>
                                    ...
                                    <form action='' method='post' style='display: inline;' onsubmit='return confirm(\"Are you sure you want to change the role of this student to lecturer?\");'>
                                        <input type='hidden' name='userID' value='" . htmlspecialchars($row["userID"]) . "'>
                                        <input type='hidden' name='changeRole' value='1'>
                                        <button type='submit' class='role-btn'><img src='img/changerole.png' alt='Change Role'></button>
                                    </form>
                                </td>
